import { CommandService, RandomService, Resource, Service, State } from "zero-ecs-lib";
import {
    Bullet, BulletType, DamageText, DamageTextType,
    ExpOrb, ExpOrbType, GameEntityType,
    Position, PositionType, Shooter, ShooterType,
    Velocity, VelocityType, Wall, WallType,
    Zombie, ZombieType,
} from "./components";
...
export class SpawnService extends Service {
    @Service.inject(CommandService) private commands!: CommandService;
    @Service.inject(RandomService) private random!: RandomService;
    @Resource.inject(GameConfigResource) private config!: GameConfigResource;
    @State.inject(GameState) private game!: GameState;

    spawnGame(): void { this.spawnShooter(); this.spawnWall(); }

    spawnShooter(): void {
        const c = this.config; const g = this.game;
        ...
        const dmgMul = 1 + (g.damageMultiplierLevel - 1) * c.upgradeDamageMultiplier;
        ...
        this.commands.spawn()
            ...
            .set(PositionType, Position.x, c.shooterX).set(PositionType, Position.y, c.shooterY)
            .add(ShooterType)
    ...
    spawnWall(): void {
        ...
        this.commands.spawn()
            .add(GameEntityType).add(PositionType)
            .set(PositionType, Position.x, c.wallX).set(PositionType, Position.y, c.wallY)
            .add(WallType).set(WallType, Wall.hp, c.wallInitialHp).set(WallType, Wall.maxHp, c.wallInitialHp)
            .submit();
    }

    spawnBullet(x: number, y: number, angle: number, damage: number, splitCount: number, ricochetCount: number, ignoreEntity = 0, isCrit = 0): void {
        const bs = this.config.bulletSpeed;
        this.commands.spawn()
            .add(GameEntityType).add(PositionType)
            .set(PositionType, Position.x, x).set(PositionType, Position.y, y)
            .add(VelocityType).set(VelocityType, Velocity.x, Math.cos(angle) * bs).set(VelocityType, Velocity.y, Math.sin(angle) * bs)
            .add(BulletType)
            .set(BulletType, Bullet.damage, damage).set(BulletType, Bullet.radius, this.config.bulletRadius)
            .set(BulletType, Bullet.speed, bs).set(BulletType, Bullet.splitCount, splitCount)
            .set(BulletType, Bullet.ricochetCount, ricochetCount).set(BulletType, Bullet.pierce, 0)
            .set(BulletType, Bullet.lifetime, this.config.bulletLifetime)
            .set(BulletType, Bullet.ignoreEntity, ignoreEntity).set(BulletType, Bullet.isCrit, isCrit)
            .set(BulletType, Bullet.active, 1)
            .submit();
    }

    private zombieStats(wave: number) {
        const c = this.config;
        return {
            hp: c.zombieBaseHp + wave * wave * 3 + wave * wave * wave * 0.04,
            speed: c.zombieBaseSpeed + wave * 3 + wave * wave * 0.5,
            xp: c.zombieBaseXp + wave * wave * 1.0,
            wallDamage: c.zombieDamageBase + wave * 0.3 + wave * wave * 0.005,
            reduction: Math.min(wave * 0.004, 0.4),
        };
    }

    spawnZombie(wave: number): void {
        const c = this.config; const s = this.zombieStats(wave);
        const y = this.random.float(c.zombieSpawnYMin, c.zombieSpawnYMax);
        this.commands.spawn()
            .add(GameEntityType).add(PositionType)
            .set(PositionType, Position.x, c.zombieSpawnX).set(PositionType, Position.y, y)
    ...
    spawnExpOrb(x: number, y: number, value: number): void {
        this.commands.spawn()
            ...
            .set(PositionType, Position.x, x).set(PositionType, Position.y, y)
            .add(VelocityType)
            .set(VelocityType, Velocity.x, -this.config.expOrbSpeed).set(VelocityType, Velocity.y, 0)
            .add(ExpOrbType)
    ...
    spawnDamageText(x: number, y: number, value: number, isCrit = 0): void {
        this.commands.spawn()
            .add(GameEntityType).add(PositionType)
            .set(PositionType, Position.x, x).set(PositionType, Position.y, y)
            .add(DamageTextType)
