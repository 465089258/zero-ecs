...
function initializationOrder<T>(
    values: readonly T[],
    registrations: ReadonlyMap<Type<T>, T>,
    key: symbol,
    ...
    for (const [type, value] of registrations) {
        const index = valueIndexes.get(value);
        if (index !== undefined) indexes.set(type, index);
    }
    values.forEach((value, index) => {
        indexes.set(Object.getPrototypeOf(value).constructor, index);
    });
...
export class BasicContainer<T> {
    ...
    /**
     * 注册已有实例，并为其具体类型到领域基类之前的原型链建立别名。
     * 同一 token 后注册的实例覆盖前者；容器锁定后会抛出错误。
     */
    set<I extends T>(type: Type<I>, instance: I): this {
        if (this.sealed) throw new Error("Container is locked");

        let current: Function | null = type;
        while (current) {
            const parent = Object.getPrototypeOf(current) as Function | null;
            if (!parent || parent === Function.prototype) break;
            this.items.set(current as Type<T>, instance);
            current = parent;
        }

        let insertionIndex = this.order.length;
        let alreadyOrdered = false;
        for (let index = this.order.length - 1; index >= 0; index--) {
            const value = this.order[index];
            if (value === instance) {
                alreadyOrdered = true;
            } else if (!this.isRegistered(value)) {
                insertionIndex = index;
                this.order.splice(index, 1);
            }
        }
        if (!alreadyOrdered) this.order.splice(insertionIndex, 0, instance);
        return this;
    }
    ...
    get<I extends T>(type: Type<I>): I {
        ...
        if (!value) throw new Error(`Instance not found: ${type.name}`);
        return value as I;
    }
    ...
    protected refreshLifecycleOrder(dynamicDependencies?: ReadonlyMap<T, ReadonlySet<T>>): void {
        this.lifecycleOrder = initializationOrder(
            this.order,
            this.items,
            this.symbol,
            dynamicDependencies,
    ...
    private isRegistered(instance: T): boolean {
