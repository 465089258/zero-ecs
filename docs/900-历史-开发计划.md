...
- 新增 [性能与分配模型](./230-规范-性能与分配.md)，逐路径记录稳定分配、高水位扩容、结构变化分配和保留边界。
...
- 增加层级 Timer、池收缩和 Listener 重入清理回归测试；详细分配边界见
