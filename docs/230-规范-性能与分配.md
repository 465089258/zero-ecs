...
- 所有 ECS 批量热路径进入一个 Chunk 后，必须先把本轮使用的组件列保存到局部变量；逐行
  循环只访问这些局部 TypedArray，不重复执行 `data[ComponentField.X][row]` 形式的组件
  列定位。该规则适用于核心包、功能模块和业务 System，不属于某个数学或渲染模块的局部优化。
...
| 正交裁剪与投影 | 无显式分配 | 相机与 Projectable Chunk 入口缓存列；逐实体只使用局部 number 和 TypedArray，视野外只写 Visible |
