...
> 状态：生效
> 权威性：规范性
> 适用范围：底层运行时、内置 Feature、第三方 Module
> 最后更新：2026-07-24
...
4. `init(context)` 初始化自身并可读取 Resource/State 和注入的 World；World 在构造时已经可用。全部 Service 完成 init 后，`activate(context)` 再建立跨 Service 连接；`start()` 使 Service 进入完整运行状态，全部 Service 启动后才执行 Startup；Shutdown 完成后再逆序调用 `stop()`；`dispose()` 释放内部结构。Service 必须在 dispose 返回前归还通过 AllocatorService 申请的 Buffer。
...
7. 生命周期 Context 是一次性受限视图，hook 返回后立即失效。不得暴露或保存原始 `InjectionContext`；长期依赖统一使用 `Inject.world/resource/state/service` 属性注入，init-only 能力可在 activate 中解析并保存窄句柄。
...
Module 只有构建期 `build()`，不会被 Game 保存，也没有 init/start/stop/dispose 钩子。
运行时能力和外部资源生命周期必须由 Module 注册的内部 Service 承担；阶段行为必须由
System 承担。Module 注册顺序不得用来表达运行时优先级。
