...
扩展包可以位于 Game 上方，但不改变三库核心分层。例如
`@zero-ecs/math -> @zero-ecs/game`、
`@zero-ecs/motion -> @zero-ecs/math -> @zero-ecs/game` 和
`@zero-ecs/flying-sword -> @zero-ecs/motion`；核心三包不得反向依赖或重导出这些扩展。
