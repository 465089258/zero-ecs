...
`FlyingSwordActionQuery` 只匹配正在执行技能的飞剑；`FlyingSwordContactQuery` 只匹配
当前开放攻击接触窗口的飞剑。内部 Storage、Motion 输入和视觉编号不从飞剑公共入口暴露。

技能开始、进入或离开接触窗口、重新入阵时，运行时通过 Commands 添加或移除瞬态动作
组件；空闲飞剑不保留技能阶段占位字段。表现层应把飞剑只读投影与自身视觉组件组成查询，
相机、投影、排序和 Canvas 均由宿主负责。
