...
数学、运动和飞剑领域实现已经收敛到 `examples/flying-sword`，只作为示例内容维护。
核心包不得导入示例，示例可以通过 Game 公共入口验证框架的组合能力。
...
公开包边界仅包含 `@zero-ecs/world`、`@zero-ecs/scheduler` 和 `@zero-ecs/game`。
`@zero-ecs/math`、`@zero-ecs/motion`、`@zero-ecs/flying-sword` 已停止发布和维护；
所需实现位于飞剑示例内部，不得从核心包重导出。详细决策见
[周边库收缩为示例实现](./395-决策-周边库收缩为示例实现.md)。
