...
仓库核心由三个锁步版本的 npm workspace 包组成，并包含两个位于 Game 上方的扩展包：
...
纯 ECS 数据内核          通用静态调度器
       ...
       DI、生命周期与标准功能
              ↑           ↑
    @zero-ecs/math    @zero-ecs/flying-sword
    ECS 原生数学组件    可选飞剑领域扩展
```

`world` 和 `scheduler` 没有框架内部依赖；`game` 通过 peer dependency 使用两者。
`math` 和 `flying-sword` 只通过 peer dependency 使用 Game 公共能力，三个核心包不反向
依赖或重导出它们。
...
飞剑领域扩展的稳定入口只有 `@zero-ecs/flying-sword` 和
`@zero-ecs/flying-sword/integration`。内部飞剑 Storage 不公开，宿主通过只读
QueryProjection 读取权威 3D 运行数据。相机、2D 投影、深度排序与绘制属于宿主表现层或
示例，不进入飞剑包。飞剑主入口同时提供数字化技能计划目录与
`FlyingSwordSkillService`；技能动作在固定 Tick 中依次经过预留、编队、制导、运动、
接触和清理阶段。
